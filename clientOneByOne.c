...
int main(int argc , char *argv[])
{
  ...
  */
  ...
  for(int i=0;i<requests;i++)
    ...
    {
      int choose,request,newval;
      do {
	printf("Please choose 1 to read, 2 to write:");
	scanf("%d",&choose);
	printf("Choose a position smaller than %d:",ARRSIZE);
	scanf("%d",&request);
	if (choose==2){
	  printf("Position:");
	  scanf("%d",&newval);
	...
	request_an_item(sock,request,newval);
	printf("The value %d is assigned to item %d\n",newval,request);
      }
